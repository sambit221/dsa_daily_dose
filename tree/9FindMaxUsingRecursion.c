// Wap to find the maximum(using recursion) element in a binary search tree.
...
int main(int argc, char const *argv[]){
    ...
    printf("Inserting few nodes(15,18,0,-1) into the tree \n");
    insert(p,15);
    insert(p,18);
    insert(p,0);
    insert(p,-1);
    printf("\nNow the min element in the tree is %d",findMin(p));
    printf("\nNow the max element in the tree is %d",findMax(p));
    return 0;
}
/* ------------output----------------
